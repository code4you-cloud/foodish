...
- 🎯 Entry points to execute your program using `python -m <foodish>` or `$ foodish` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/revego/foodish/branch/main/graph/badge.svg?token=foodish_token_here)](https://codecov.io/gh/revego/foodish)
[![CI](https://github.com/revego/foodish/actions/workflows/main.yml/badge.svg)](https://github.com/revego/foodish/actions/workflows/main.yml)

Awesome foodish created by revego

## Install

from source
```bash
git clone https://github.com/revego/foodish foodish
cd foodish
make install
```
...
```bash
$ foodish run --port 8080
```

or

```bash
python -m foodish run --port 8080
```
...
```bash
❯ foodish --help
Usage: foodish [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ foodish create-user --help
Usage: foodish create-user [OPTIONS] USERNAME PASSWORD
...
```bash
foodish create-user admin admin --superuser
```
...
Run with `foodish run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `foodish create-user`.
...
-----------------------------------------------------
foodish/__init__.py              4      0   100%
foodish/app.py                  16      1    94%
foodish/cli.py                  21      0   100%
foodish/config.py                5      0   100%
foodish/db.py                   10      0   100%
foodish/models/__init__.py       0      0   100%
foodish/models/content.py       47      1    98%
foodish/routes/__init__.py      11      0   100%
foodish/routes/content.py       52     25    52%
foodish/routes/security.py      15      1    93%
foodish/routes/user.py          52     26    50%
foodish/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in foodish/default.toml.
